...
CREATE TABLE IF NOT EXISTS wastage_requests (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    property_id UUID NOT NULL REFERENCES properties(id),
    status wastage_status DEFAULT 'pending',
    reason TEXT,
...
CREATE TABLE IF NOT EXISTS wastage_items (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    wastage_id UUID NOT NULL REFERENCES wastage_requests(id) ON DELETE CASCADE,
    item_id UUID NOT NULL REFERENCES items(id),
    quantity NUMERIC(12, 4) NOT NULL CHECK (quantity > 0),
    reason_detail TEXT
);
...
CREATE TRIGGER trg_process_wastage_authorization
...
BEGIN
    IF TG_OP = 'UPDATE' OR TG_OP = 'DELETE' THEN
        RAISE EXCEPTION 'Audit logs are immutable. Direct % prohibited.', TG_OP;
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp;
